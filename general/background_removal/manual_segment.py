...
class App():
    ...
    def run(self,filename):
        ...
        cv.moveWindow('output', 1000, 100)
        #cv.moveWindow('input', 400, 500)
...
if __name__ == '__main__':
    ...
    num_of_classes = 1
    train_valid_dirs = ['train', 'valid']
    output_folder = 'background_removed'
    input_data_main_dir = '../../data_loader/data/site_period_top_200/'
    ...
    for dir_ in train_valid_dirs:
        ...
        for k in range(num_of_classes):
            outdirName = output_folder + '/' + dir_ + '/' + str(k)
            if not os.path.exists(outdirName):
                os.mkdir(outdirName)

            cur_input_dir = input_data_main_dir + '/' + dir_ + '/' + str(k)
            for filename in os.listdir(cur_input_dir):
                if filename.endswith(".jpg"):
                    App().run(cur_input_dir + '/' + filename)
                    #img = cv.imread(cur_input_dir + '/' + filename)
